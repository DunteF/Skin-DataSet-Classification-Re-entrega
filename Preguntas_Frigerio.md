...
- ¿Por qué convertimos las imágenes a `ToTensorV2()` al final de la pipeline?
   ...
   ToTensorV2 convierte las imágenes de Arrays Numpy en Tensores Pytorch.  Además de trabajar objetos distintos, Albumentations trabaja con arrays Numpy en formato channel last, mientras que Pytorch con tensores channel first.
...
- ¿Por qué usamos una red MLP en lugar de una CNN aquí? ¿Qué limitaciones tiene?
   ...
   Una red MLP tiene la limitación de precisar de un flatten para interpretar las imagenes, esto pierde mucha riqueza de información por deshacer las relaciones espaciales entre pixeles. Las CNNs en cambio conservan las estructuras espaciales.
...
- ¿Cómo afecta la elección del tamaño de batch (`batch_size`) al entrenamiento?
   ...
   El batch size afecta cuan ruidoso es el entrenamiento. A mayor batch size se promedia la loss de más datos para calcular el gradiente en cada actualización de pesos, esto hace que el modelo converja con menos ruido. 
...
- ¿Qué significa una accuracy del 70% en validación pero 90% en entrenamiento?
   ...
   Significa que hay overfitting a los datos de entrenamiento, el modelo no generaliza a datos nuevos. Sería pertinente  ajustar las técnicas de regularización o conseguir más datos por aumentación o ampliación del dataset.

- ¿Qué otras métricas podrían ser más relevantes que accuracy en un problema real?

   En algunos problemas reales la sensibilidad (recall) puede ser más relevant que la precisión (accuracy). Por ejemplo en screening médico el costo de un falso negativo es mucho mayor que el de un falso positivo, porque un falso positivo va ser revisado por un médico mientras que el falso negativo no será detectado y podría evitar una intervención que sería beneficiosa para el paciente. También podría ser muy relevante la matriz de confusión dado que puede informar acerca de posibles diagnosticos alternativos a la predicción de la red. Esto podría ser importante de considerar, por ejemplo si la red suele confundir dos clases, una para una enfermedad mortal y una para una enfermedad quasi benigna o leve sería criterioso hacer más estudios para descartar la posibilidad de la enfermadad mortal incluso si la predicción de la red indica que el caso benigno es el más probable.
...
- En el reporte de clasificación, ¿qué representan `precision`, `recall` y `f1-score`?
   ...
   Precisión es el valor predictivo positivo, es decir el porcentaje de las predicciones positivas que fueron acertadas. En este caso se puede ver en la matriz de confusión como el valor de la diagonal sobre la suma de su columna. 
...
- ¿Qué problema podríamos tener si entrenamos este modelo con muy pocas imágenes por clase?
   ...
   El modelo tendría muy pocos datos para la cantidad de parametros que tiene (6.358.922), posiblemente haya overfitting.

- ¿Cómo podríamos adaptar este pipeline para imágenes en escala de grises?

   U  nicamente habría que instanciar el modelo con input_size=64*64*1 y adaptar  la clase de datasets para que convierta las imagenes a Numpy leyendolas como greyscale en lugar de RGB. 
...
3. Aplicar Weight Decay (L2):
   - Modificar el optimizador:
     ...
     Observamos que el train accuracy mejora considerablemente, los valores en validación permanencen casi iguales.
...
4. Reducir overfitting con data augmentation:
   ...
   Se emplearon desde el comienzo con train_transform que emplea estás 3 transformaciones.
...
Podrían producirse simetrías que no se rompan y se pierda mucha de la capacidad de generalización y expresividad de la red.

- ¿Cuál es la diferencia entre las inicializaciones de Xavier (Glorot) y He?

   EL criterio de Glorot trata de homogeneizar la varianza en la entrada y salida de una capa, con el objetivo de evitar una explosión o desvanecimiento de la señal por acción de la distribución de los pesos. Con este propósito inicializa los pesos de manera aleatoria con una distribución cuya desviación estandar es proporcional a la raíz de la inversa de la suma de la cantidad de entradas y salidas de la capa (Normal Glorot, puede ser uniforme donde este valor calculado con #entradas y #salidas son los extremos de la distribucion).Se usa priuncipalmente para funciones de activación simétricas como sigmoidea o tanh. 
   He se usa con el mismo propósito para funciones de activación asimétricas como ReLU. El escalado de la varianza de salida debido a los pesos se restringe con una distribución cuya varianza está vinculada con la raiz de la inversa de la entrada unicamente.
![alt text](image.png)

- ¿Por qué en una red con ReLU suele usarse la inicialización de He?

   Porque He usa una distribución cuya  varianza  es proporcional a 2/Nin, que es mayor que la de Glarot, la cual es proporcional a 2/(Nin+Nout). Esto ayuda a compensar la varianza perdida por la rama anulada por  la ReLU.
